({
	getTemplate : function(component, event, helper) {
        ...
        if (component.get("v.sObjectName") == 'buildertek__Change_Order__c') {
            ...
            folname = 'BT PO';
        } else if (component.get("v.sObjectName") == 'buildertek__Service_Request__c') {
            folname = 'BT SR';
        }
        else{
        ...
        dbAction.setCallback(this, function (response) {
            ...
            if (state === "SUCCESS" && response.getReturnValue().length > 0) {
                ...
                console.log('selectedTemplate: ',component.get("v.selectedTemplate"));
            }
        });
    ...
    getContactList : function(component, event, helper) {
        var action = component.get("c.getMultipleContact");
        action.setParams({
            "recordId" : component.get("v.recordId"),
            "objectAPIName" : component.get("v.sObjectName")
        });
        action.setCallback(this, function(response){
            ...
            if(state === "SUCCESS"){
                ...
                console.log('Error in getContactList', response.getError());
            }
        });
        $A.enqueueAction(action);
    },
    ...
    getCoInfo :function(component,event,heplper){
        var action = component.get("c.getCompanyInfo");
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {
                // Set the companyInfo attribute with the response
                component.set("v.companyInfo", response.getReturnValue());
                var coInfo = component.get("v.companyInfo");
                let purchaseOrderBody = '';
                let greeting = 'Hello recipient,';
                if (component.get("v.sObjectName") == 'buildertek__Purchase_Order__c') {
                    purchaseOrderBody = 'Attached is your Purchase Order.';
                    greeting = 'Hello Vendor,';
                }
                let emailBody = `${greeting}<br><br>${purchaseOrderBody}<br><br><br>Sincerely,<br>${coInfo.Name || ''}<br>${coInfo.Address.street || ''}<br>${coInfo.City || ''}, ${coInfo.State || ''} ${coInfo.PostalCode || ''}<br>${coInfo.Phone || ''}`;
                component.set("v.templateBody", emailBody)
            }
        });
        $A.enqueueAction(action);
    },
