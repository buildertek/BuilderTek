({
    ...
    taskdrawTable: function(component, event, helper) {
        ...
        for(var i=0;i<PaginationList.length;i++){
            ...
            // Start creating the text to display on UI
            let displayText = PaginationList[i].title;

            // Handle `projectId`
            let projectId = PaginationList[i].projectId ? PaginationList[i].projectId : 'No Project';

            // Collect and validate additional fields
            let resourceFields = [];
            if (PaginationList[i].ContractorName) resourceFields.push(PaginationList[i].ContractorName);
            if (PaginationList[i].ContractorName1) resourceFields.push(PaginationList[i].ContractorName1);
            if (PaginationList[i].internelresource) resourceFields.push(PaginationList[i].internelresource);
            if (PaginationList[i].internelresource1) resourceFields.push(PaginationList[i].internelresource1);

            // Join additional fields (or display "None" if none are valid)
            let resourceFieldsText = resourceFields.length > 0 ? resourceFields.join(', ') : '';

            // Create the full display text
            let fullText;
            if(resourceFieldsText != ''){
                fullText = `${projectId} - ${resourceFieldsText}`;
            } else {
                fullText = `${projectId}`;
            }

            tbody += `<td style="border-right: 1px solid #ECECEC !important;width: 335px;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;display: inline-block;" title="${displayText} (${fullText})">`
                + '<image src="/resource/buildertek__TaskIcon" style="width: 35px;height: 35px;border-radius: 50%;text-align: center;position: absolute;margin-top: 4px;" />'
                + '&nbsp;&nbsp;<b>';

            // Add the `<a>` tag for the clickable link
            tbody += `<a style="margin-left:31px;" target="_blank" href="/${PaginationList[i].Id}">${displayText}</a></b><br/>`;

            // Append the text to display on the UI
            tbody += `&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; (${fullText})</td>`;
                ...
                if( conflictview != 'Conflicts'  ){
                    ...
                    if( new Date(PaginationList[i].endday).valueOf() <=  new Date(weekDates[k].Date).valueOf() && new Date(todaydate).valueOf() >= new Date(weekDates[k].Date).valueOf()){
                        ...
                        if(businessworkingdys == 0){
                            ...
                            datatable+='<tbody><tr>';
                            datatable+='<th style="border: 1px solid #ddd;font-size: 12px;padding: 3px;">Vendor Resource</th>';
                            datatable+='<th style="border: 1px solid #ddd;font-size: 12px;padding: 3px;">Vendor Resource 1</th>';
                            datatable+='<th style="border: 1px solid #ddd;font-size: 12px;padding: 3px;">Internal Resource</th>';
                            datatable+='<th style="border: 1px solid #ddd;font-size: 12px;padding: 3px;">Internal Resource 1</th>';
                            ...
                            if(PaginationList[i].ContractorName1 != undefined){
                                datatable+='<td style="border: 1px solid #ddd;font-size: 12px;padding: 3px; class="slds-truncate"">'+PaginationList[i].ContractorName1 +'</td>';
                            }else{
                                datatable+='<td style="border: 1px solid #ddd;font-size: 12px;padding: 3px; class="slds-truncate"></td>';
                            }
                            ...
                            if(PaginationList[i].internelresource1 != undefined){
                                datatable+='<td style="border: 1px solid #ddd;font-size: 12px;padding: 3px; class="slds-truncate"">'+PaginationList[i].internelresource1 +'</td>';
                            }else{
                                datatable+='<td style="border: 1px solid #ddd;font-size: 12px;padding: 3px; class="slds-truncate"></td>';
                            }
