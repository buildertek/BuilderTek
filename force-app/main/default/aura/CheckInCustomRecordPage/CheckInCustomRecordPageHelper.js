        ...
        try {
            ...
            action.setCallback(this, function (res) {
                ...
                if (state == "SUCCESS") {
                    ...
                    console.log('Has prev'+result.hasprev);
                    console.log('Result offset'+result.offst);
                    ...
                    var aCategoryPicklistOptions = component.get('v.categoryOptions');
                    var categoryPicklistOptions = [];
                    result.categoryOptions.forEach(function(option) {
                        categoryPicklistOptions.push({
                            label: option,
                            value: option
                        });
                    });
                    categoryPicklistOptions.unshift({
                        label: 'All',
                        value: 'All'
                    });
                    if (aCategoryPicklistOptions.length == 0) {
                        component.set('v.categoryOptions', categoryPicklistOptions);
                    }
                    var aStatusPicklistOptions = component.get('v.statusOptions');
                    var statusPicklistOptions = [];
                    result.statusOptions.forEach(function(option) {
                        statusPicklistOptions.push({
                            label: option,
                            value: option
                        });
                    });
                    statusPicklistOptions.unshift({
                        label: 'All',
                        value: 'All'
                    });
                    //aStatusPicklistOptions 
                    if (aStatusPicklistOptions.length == 0) {
                        component.set('v.statusOptions', statusPicklistOptions);
                    }
