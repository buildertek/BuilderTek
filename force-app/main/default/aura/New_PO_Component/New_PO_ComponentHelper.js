({
	...
	fetchpricebooks: function (component, event, helper) {
		var actions = component.get("c.getPricebooksWrapper");
	...
	updateProductFamilyOptions: function (component, index) {
		var listofPOItems = component.get("v.listofPOItems");
		var pricebookName = listofPOItems[index].pricebookName;

		var action = component.get("c.getProductfamily");
		action.setParams({
			"ParentId": pricebookName
		});
		...
		action.setCallback(this, function (response) {
			...
			if (state === "SUCCESS") {
				...
				listofPOItems[index].productfamilyOptions = storeResponse;

				if (storeResponse.length > 0) {
					listofPOItems[index].productfamily = storeResponse[0].productfamilyvalues;
				} else {
					listofPOItems[index].productfamily = '';
				}
