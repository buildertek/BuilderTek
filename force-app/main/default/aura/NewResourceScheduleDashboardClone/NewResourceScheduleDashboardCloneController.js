({
    ...
    monthView: function (component, event, helper) {
        try {
            console.log("Month view invoked");
            ...
            console.log('Error in month view --->', error.stack);
            
        }
    },

    weekView: function (component, event, helper) {
        try {
            console.log("Week view invoked");
            document.getElementById('mycalendar2').style.display = 'none';            
            document.getElementById('calendar').style.display = 'block';
            ...
            console.log('Error in weekview --->', error.stack);
    ...
    prevDay: function (component, event, helper) {
        try {
            console.log("prevday invoked");
            $('#calendar').fullCalendar('prev');
            /*Show day view div*/
            
        } catch (error) {
            console.log('Error in  prevday --->', error.stack);
    ...
    nextDay: function (component, event, helper) {
        try {
            console.log("nextday invoked");
            $('#calendar').fullCalendar('next');

            /*Show day view div*/
            
        } catch (error) {
            console.log('Error in nextday --->', error.stack);
    ...
    today: function (component, event, helper) {
        try {
            ...
            console.log('Error in today --->', error.stack);
            
        }
    },

    toggle1: function (component, event, helper) {
        console.log('toggle 1 invoked');
        
        const sidebar = document.getElementById("sidebar1");
        const toggleButton = document.getElementById("toggleButton1");
        sidebar.classList.toggle("collapsed");
        const thing1 = document.querySelector('.left-nav-main .px-3');
        console.log(thing1);

        const thing2 = document.querySelector('.left-nav-main .link-dark');
        console.log(thing2);
        
        // Toggle the arrow direction
        const icon = toggleButton.querySelector("i");
        if (sidebar.classList.contains("collapsed")) {
            console.log('contains collapsed');
            thing1.style.opacity = '0';
            thing2.style.opacity = '0';
            icon.classList.remove("fa-chevron-left");
            icon.classList.add("fa-chevron-right");
        } else {
            console.log('in else');
            thing1.style.opacity = '1';
            thing2.style.opacity = '1';
    ...
    toggle2: function (component, event, helper) {
        console.log('toggle 2 invoked');
        
        const sidebar = document.getElementById("sidebar2");
        const toggleButton = document.getElementById("toggleButton2");
        sidebar.classList.toggle("collapsed");
        const thing1 = document.querySelector('.left-nav2 .px-3');
        console.log(thing1);
        
        const thing2 = document.querySelector('.left-nav2 .list-group');
        console.log(thing2);
    ...
    handleSelectedProject :function (component, event, helper) {
        ...
        component.set("v.isFetchingNeeded", false);
    ...
    selectedResource :function (component, event, helper) {
        try {
            ...
            console.log(event.currentTarget);
            var activeEle = document.getElementsByClassName('list-group-item activeResource');
            if(activeEle){
                for( let a of activeEle){
                    a.classList.remove('activeResource');
                }
            }
            toggleText.blur();
            ...
            if(resources.length){
                ...
                action.setCallback(this, function (response) {
                    ...
                    if(state === "SUCCESS"){
                        ...
                        component.set("v.areExternalResource", response.getReturnValue().areExternalResource);
                        component.set("v.areInternalResource", response.getReturnValue().areInternalResource);
                        component.set("v.isSelectedResource", true);
    ...
    doTaskResourceSearch: function (component, event, helper) {
        try {
            ...
            var startDate = component.get("v.startrange");
            var endDate = component.get("v.endrange");
            ...
            var action = component.get("c.getScheduleItemsByProject");
                ...
                action.setCallback(this, function (response) {
                    ...
                    if(state === "SUCCESS"){
                        ...
                        document.getElementById('selectedContractResource').innerText = 'Resource';
                        document.getElementById('selectedContractResourceTradeType').innerText = 'Trade Type';
                        console.log('reached till calendar');
    ...
    doTaskAllFilterSearch:  function (component, event, helper) {
        ...
        action.setCallback(this, function (response) {
            ...
            if(state === "SUCCESS"){
                ...
                component.set("v.eventList", evetList);
                console.log('logging eventlist--->'+evetList);
    ...
    setConflictData:  function (component, event, helper) {
        try {
            ...
            var weekDates = component.get('v.daysInMonth');
            ...
            var taskIds = [];
            console.log('component.get("v.dateval")-->'+component.get("v.dateval"));
            console.log(weekDates);
            
            for (var i = 0; i < eventList.length; i++) {
                for (var k = 0; k < weekDates; k++) {
                    if (eventList[i].ProjectTaskRecordsList != undefined) {
                        ...
                        console.log('Date eval',component.get('v.dateval'));
                        ...
                        if(tasks >= 2 && eventList[i].simultaneousTasksContractorResources != undefined && tasks < eventList[i].simultaneousTasksContractorResources){
                            ...
                            for (var t = 0; t < EquipmentRecordsList.length; t++) {
                                console.log('aboveif----calandaedate--->'+'record stardate>='+ EquipmentRecordsList[t].day +'record enddate-----&& <= '+EquipmentRecordsList[t].endday);
                                ...
                                if (new Date(dateVal.getFullYear(),dateVal.getMonth(),k+1).valueOf() >= new Date(EquipmentRecordsList[t].day).valueOf() && new Date(dateVal.getFullYear(),dateVal.getMonth(),k+1).valueOf() <= new Date(EquipmentRecordsList[t].endday).valueOf()) {
                                    console.log('belowif----calandaedate--->'+'record stardate>='+ EquipmentRecordsList[t].day +'record enddate-----&& <= '+EquipmentRecordsList[t].endday);
                                    console.log('555')
    ...
    afterScriptsLoaded: function (component, event, helper) {
        try {
            ...
            console.log('error in afterScriptsLoaded : ', error);
        }
