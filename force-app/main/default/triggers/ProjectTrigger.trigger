trigger ProjectTrigger on buildertek__Project__c (after insert,after update , before delete, after delete) {
    ...
    System.debug('2nd Condition ==> ' + ProjectTriggerHandler.blnSkipProjectTrigger);
     if(!BT_Utils.isTriggerDeactivate('Project__c') && !ProjectTriggerHandler.blnSkipProjectTrigger){
        ...
        list<buildertek__Admin_Interface_Node_Configuration__c> adminInterfaceNodeConfigurationslist = [Select Id,buildertek__Retreive_Aerial_Photo_of_project__c,buildertek__Google_Map_API_Key__c, buildertek__CustomGeoLocation__c from buildertek__Admin_Interface_Node_Configuration__c WHERE Name=:'Project Configuration' limit 1];
        ...
        if(Trigger.isInsert && Trigger.isAfter){
            ...
            if(adminInterfaceNodeConfigurationslist.size()>0 && adminInterfaceNodeConfigurationslist[0].buildertek__CustomGeoLocation__c == true){
                handler.createDummyAccountForGeoLocation(Trigger.new, Trigger.oldMap, true);
            }
        }
            ...
            if(adminInterfaceNodeConfigurationslist.size()>0 && adminInterfaceNodeConfigurationslist[0].buildertek__CustomGeoLocation__c == true){
                handler.createDummyAccountForGeoLocation(Trigger.new, Trigger.oldMap, false);
            }
        }
