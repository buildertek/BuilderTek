...
export default class CreateNewSchedule extends NavigationMixin(LightningElement) {
    ...
    createSchedule() {
        ...
        else if(this.initialStartDate == null || this.initialStartDate == undefined || !dateFormatRegex.test(this.initialStartDate)){
            const event = new ShowToastEvent({
                title: 'Error',
                message: 'Please enter an Initial Start Date',
                variant: 'error',
                mode: 'dismissable'
            });
            this.dispatchEvent(event);
