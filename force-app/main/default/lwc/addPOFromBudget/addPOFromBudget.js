...
export default class AddPOFromBudget extends LightningElement {
    ...
    UpdatePO(event) {
        ...
        if (event.target.dataset.name === 'PO') {
            let poListOfID = this.selectedRows.map(obj => obj.Id);
            updatePOFromBudget({ POId: poListOfID, budgetId: this.budgetId, BudgetLineId: this.budgetLineId })
                .then(() => {
            ...
            console.log('poItemListOfID', poItemListOfID);
