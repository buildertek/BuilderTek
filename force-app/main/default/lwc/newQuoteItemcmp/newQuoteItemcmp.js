...
export default class NewQuoteItemcmp extends NavigationMixin(LightningElement) {
    ...
    @track isCreateScheduleTrue = false;
    @track fields = {
        buildertek__Description__c: '',
        buildertek__Grouping__c: '',
        buildertek__Category__c: '',
        buildertek__Option__c: '',
    ...
    getData() {
        ...
        getallData({ quoteId: QuoteId })
            .then(result => {
                ...
                this.fieldsFromFieldSet.map(fl => {
                    if (fl.name == 'buildertek__Grouping__c') {
                        fl.option = groupingOption;
                        fl.value = this.fields.buildertek__Grouping__c;
                    }
                });
    ...
    calculateTotal(data) {
        ...
        this.data.forEach(item => {
            ...
            const { buildertek__Net_Unit__c, ...subTotalWithoutNetUnit } = subTotal;
            item.items.push(subTotalWithoutNetUnit);
            subtotalList.push(subTotalWithoutNetUnit);
        ...
        // Destructure grandTotal to remove 'buildertek__Net_Unit__c'
        const { buildertek__Net_Unit__c, ...grandTotalWithoutNetUnit } = grandTotal;
        this.grandTotalList = [JSON.parse(JSON.stringify(grandTotalWithoutNetUnit))];
    ...
    getSelectedLines(event) {
        try {
            ...
            let selectedRows = event.detail.selectedRows; // Rows selected in the table
            console.log({ selectedRows });
            
            // Store selected rows under the corresponding dataset id
            this.selectedLineObj[event.target.dataset.id] = selectedRows;
            
            // Merge all selected rows into one array
            Object.keys(this.selectedLineObj).forEach(key => {
                this.selectedLineObj[key].forEach(row => {
                    // Only add rows where Name is not "Subtotal"
                    if (row.Name !== "Subtotal") {
                        selectedTempTableData.push(row);
                    }
                });
            });
    
            // Assign merged data to the component variable
            this.selectedTableData = selectedTempTableData;
    
            console.log('selectedTempTableData ==> ', this.selectedTableData);
        } catch (error) {
            console.error('Error in getSelectedLines: ', error);
        }
    }    
    ...
    handleCreateSchedule(event) {
        ...
        if (this.selectedTableData.length > 0) {
            this.isCreateScheduleTrue = true;
        } else {
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Warning',
                    message: 'Please select at least one Quote Line to Create Schedule',
                    variant: 'warning'
                })
            );
        }
    ...
    closeCreateSchedule(event){
        this.isCreateScheduleTrue = false;
        if (event.detail.refresh) {
            this.refreshData();
        }
    }
