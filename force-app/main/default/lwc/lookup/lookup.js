...
import getProducts from "@salesforce/apex/LookupController.getProducts";
import getTasksFromProject from "@salesforce/apex/LookupController.getTasksFromProject";
...
export default class Lookup extends LightningElement {
  ...
  @api pricebookId; // Price Book Id (Optional)
  @api productFamily; // Product Family (Optional)
  @api pid = [];
  @api showLabel = false; // Show Label (Optional)
  ...
  handleSearch(searchTerm, selectedIds) {
    ...
    if(this.pricebookId == undefined && this.productFamily == undefined){
      const parameters = {
      ...
      console.log('in else');
      getProducts({objectName: this.objectApiName, pricebookId: this.pricebookId, productFamilyName: this.productFamily, searchTerm: searchTerm})
        .then((results) => {
          console.log(results);
          this.searchResults = results;
          this.noRecordFound = results.length == 0;
  ...
  handleProjectSearch(searchTerm, selectedIds){
    console.log('in handleProjectSearch :: ', this.pid);
      getTasksFromProject({objectName: this.objectApiName, searchTerm: searchTerm, projectIds: this.pid, selectedIds: selectedIds })
        .then((results) => {
          console.log(results);
          this.searchResults = results;
          this.noRecordFound = results.length == 0;
